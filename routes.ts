...
router
    ...
    .post("/login", async function({ req, store, db }) {
        ...
        if (!email || !password) {
            throw new AuthValidationError("You must be signed in with a username before you can chat!");
        }

        const user = await db.findUser(email, password);
        if (!user) {
            throw new AuthValidationError("The email and/or password provided is not valid. Please try again.");
        }
        ...
        const token = jwt.sign({ userId: user.id }, String(import.meta.env.JWT_SECRET));
    ...
    .post("/register", async function({ req, db }) {
        ...
        if (!username || !password || !email) {
            throw new ValidationError("You must provide an email, username and password to register!");
        }
    ...
    .post("/logout", function({ req }) {
        ...
        const { userId } = jwt.verify(parsedCookies.token, String(import.meta.env.JWT_SECRET), {
            maxAge: 2592000
        }) as JWTUserId;
    ...
    .get("/chat", async function({ req, server, store }) {
        ...
        if (!token) {
            throw new ValidationError("User attempted to connect with an empty token.");
        }
        ...
        if (!username) {
            throw new ValidationError("User attempted to connect with an invalid token.");
        }
    ...
    .static("assets", ({ req }) => new Response(Bun.file(`./chat-client/dist${req.URL.pathname}`)))
    .static("chat.svg", ({ req }) => new Response(Bun.file(`./chat-client/dist${req.URL.pathname}`)));
